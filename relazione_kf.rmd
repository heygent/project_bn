...
library(reticulate)
use_python("/home/sinopeta/Scrivania/IA/Torta/Progetto/venv/bin/python")
```
...
`calculate_movement` è la legge del moto accelerato sotto forma matriciale, dove il parametro `control` sta a
indicare l'accelerazione dell'oggetto. La usiamo sia per fare le predizioni all'interno del Kalman Filter, sia
...
La funzione quindi dà in output lo stato reale, la misurazione effettuata, e la corrispondente predizione fatta dal Kalman Filter, 
...
Nelle ultime fasi invece vediamo che misurazioni molto sbilanciate fanno cambiare di poco la stima, come nel caso della velocità verso l'istante 25.
