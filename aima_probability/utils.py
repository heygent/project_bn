...
def histogram(values, mode=0, bin_function=None):
    ...
    if mode:
        return sorted(
            list(bins.items()), key=lambda x: (x[1], x[0]), reverse=True
        )
    else:
...
def print_table(table, header=None, sep="   ", numfmt="{}"):
    ...
    table = [
        [numfmt.format(x) if isnumber(x) else x for x in row] for row in table
    ]
...
def ucb(n, C=1.4):
    return (
        float("inf")
        if n.N == 0
        else n.U / n.N + C * math.sqrt(math.log(n.parent.N) / n.N)
    )
